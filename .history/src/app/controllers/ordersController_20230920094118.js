...
class orderController{
    ...
    async deleteOrder(req,res){
        ...
        .catch(error => res.status(400).json({Success :'false',Message : 'Error deleting order',error}))
    ...
    //         if(deleteOrder)
    //         {
    //             await Promise.all(deleteOrder.orderItems.map(async (bien) => {
    //                 await OrderItems.findByIdAndRemove(bien)}))
    //         }
    //         else{
