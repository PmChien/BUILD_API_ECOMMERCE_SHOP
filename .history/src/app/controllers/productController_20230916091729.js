...
class productController {
    ...
    async showFeaturedProduct (req,res)
    {
        const countFeaturedProduct = await product.countDocuments({isFeatured : true})
        const count = req.params.count ? req.params.count : 0
        const featuredProduct = await product.findOne({isFeatured : true}).limit(+count)
        if(!featuredProduct || !countFeaturedProduct)
        {
            return res.status(404).send('Product not found')
        }
        try {
            res.status(200).json({Success :'true',Total: countFeaturedProduct,featuredProduct})
        } catch (error) {
            console.log(error)
