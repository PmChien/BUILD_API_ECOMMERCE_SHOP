...
class orderController{
    ...
    // GET  get /countOrders count all orders
    async countOrders(req, res) {
        try {
            const countOrders = await order.countDocuments()
            if(!countOrders || countOrders.length === 0) {
                return res.status(400).json({ Message: 'Error count orders' });
            }
            res.status(200).json({ Message:'Success' ,total :`there are ${countOrders} orders`});
        } catch (error) {
            res.status(500).json({ Message: 'Internal Server Error' ,error});
        }
      }
    // GET  get /countOrders count all orders of user '/get/countUserOrders/:userId'
    async countUserOrders(req,res){
        try {
            const ordersUser = await order.findOne({ user : req.params.userId})
            .populate({ path : 'orderItems', populate :{path : 'product',populate :'category'}})
            .sort({ 'dateOrdered' : -1})
            if(!ordersUser) {return res.status(404).json({ Message:'The user has not placed any orders yet'}); }
            res.status(200).json({ Message :'founded',ordersUser});
        } catch (error) {
            res.status(500).json({ Message: 'Internal Server Error' ,error});
        }
    }
